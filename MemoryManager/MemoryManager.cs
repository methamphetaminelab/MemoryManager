...
#if WINXP
#else
        public UIntPtr CreateCodeCave(UIntPtr address, byte[] newBytes, int replaceCount, int size = 0x1000)
        {
            if (replaceCount < 10)
                return UIntPtr.Zero;

            UIntPtr caveAddress = UIntPtr.Zero;
            UIntPtr preferred = address;
            for (int i = 0; i < 10 && caveAddress == UIntPtr.Zero; i++)
            {
                caveAddress = VirtualAllocEx(mProc.Handle,
                    FindFreeBlockForRegion(preferred, (uint)size),
            ...
            if (caveAddress == UIntPtr.Zero)
            {
                caveAddress = VirtualAllocEx(mProc.Handle, UIntPtr.Zero, (uint)size,
                                             MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
            }

            int offsetToCave = (int)((long)caveAddress - (long)address - 5);
            byte[] jmpToCave = new byte[5];
            jmpToCave[0] = 0xE9;
            BitConverter.GetBytes(offsetToCave).CopyTo(jmpToCave, 1);
            WriteBytes(address, jmpToCave);

            byte[] fixedPatch = new byte[5] { 0x0F, 0x1F, 0x00, 0xEB, 0x67 };
            WriteBytes(UIntPtr.Add(address, 5), fixedPatch);

            byte[] caveBytes = new byte[newBytes.Length + 5];
            newBytes.CopyTo(caveBytes, 0);
            int offsetBack = (int)(((long)address + replaceCount) - ((long)caveAddress + newBytes.Length + 5)) - 0xC;
            caveBytes[newBytes.Length] = 0xE9;
            BitConverter.GetBytes(offsetBack).CopyTo(caveBytes, newBytes.Length + 1);
